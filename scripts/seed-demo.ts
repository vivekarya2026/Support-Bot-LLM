...
import { createBot, getBotBySlugAsync } from "../lib/bots";
...
async function main() {
  let bot = await getBotBySlugAsync("demo");
  if (!bot) {
    bot = await createBot({ name: "Demo Bot", templateId: "saas-support" });
    ...
    await resetKnowledgeBase(bot.id);
  }
